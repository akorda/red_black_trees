...
\begin{scope}[every node/.style={circle, minimum size=14, inner sep=0, font=\small,thick,draw}]
    ...
    \node (s) [label=right:h+2] at (1.00, -1.50) {s};
...
\begin{scope}[every node/.style={circle, minimum size=14, inner sep=0, font=\small,thick,draw}]
    \node (s) [label=left:h] at (0.00, 0.00) {s};
    \node (p) [label=left:h+1] at (-1.50, -1.50) {p};
    \node (v) [label=left:h+2] at (-3.00, -3.00) {v};
    \node (s1) [label=right:h+1] at (-0.00, -1.50) {s1};
    \node (s2) [label=right:h+1] at (1.50, -1.50) {s2};
\end{scope}
\begin{scope}[>={Stealth[black,length=3mm]},
              every node/.style={fill=white,circle},
              red_edge/.style={draw=red,very thick},
